                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {EDUCATIONS.map((edu, index) => (
                        <div key={index} className="bg-white shadow-md p-6 rounded-2xl flex flex-col items-start text-left ">
                            <h2 className="text-xl md:text-2xl text-black font-medium mb-2">{edu.degree}</h2>
                            <h3 className="text-xs md:text-sm text-gray-700">{edu.institution}</h3>
                            <p className="text-[0.6rem] md:text-xs text-gray-400 ">{edu.date}</p>
                            <ul className="list-disc list-outside ms-5 mt-5">
                                {edu.description.map((desc, index) => {
                                    return <li key={index} className="text-sm md:text-base text-gray-700">{desc}</li>
                                })}
                            </ul>
                        </div>
                    ))}
